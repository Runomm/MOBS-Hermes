...
### 🆕🍎 İLK iOS CİHAZ TESTİ — iPhone 17 Pro Max (2026-06-14, Mehmet birebir)

> "indirme işlevleri arka planda devam etmeli ve iptal et butonu da olmalı.
>
> görsel çeviri için ios'de fotoğraf çek ve galeri butonları tepki vermiyor izin de istemiyor.
>
> manuel çeviri modunda türkçe ve ingilizce ml kit'in ikiside yüklü olmasına rağmen çevir butonu tepki vermiyor/çeviri yapmıyor.
>
> smallTalk modunda mikrofon izni reddedildi yazıyor ve başlamıyor ancak izin istemedi bile ? aynı şekilde konferans modunda çalışmıyor mikrofon izni yok diyor
>
> bütün bunlar yetmezmiş gibi cihaz aşırı ısınıyor ki henüz nllb'i veya gemma indirmedim bile"

**Bağlam:** Hermes ilk kez iPhone 17 Pro Max'te çalıştı (Codemagic imzasız IPA → Sideloadly ücretsiz Apple ID → Developer Mode). Açılış + Home + navigasyon ÇALIŞIYOR. Yukarıdaki 5 madde ilk iOS fonksiyonel testinin bulguları.

**Analiz / yapılacaklar (iOS):**
- **(i1) İndirmeler arka plan + iptal:** Ayarlar indirme akışı foreground/await; arka plan devamı + İptal butonu istenen. (feature)
- **(i2) Görsel Çeviri kamera/galeri tepkisiz + izin istemiyor:** image_picker iOS — buton hiç tepki vermiyor (Android'de çalışıyor). Info.plist usage string'leri eklendi (build'de var). Olası: buton sessiz exception / image_picker_ios sorunu. İNCELENECEK.
- **(i3) Manuel çeviri Çevir tepkisiz (TR+EN ML Kit yüklü):** ML Kit translate iOS'ta çevirmiyor/buton tepkisiz. ML Kit on-device iOS model durumu/exception. İNCELENECEK.
- **(i4) Mikrofon izni — istemeden "reddedildi":** SmallTalk + Konferans mic iznini SORMADAN denied diyor. Olası kök neden: iOS'ta ilk durum `denied`/`notDetermined`; kod `request()` çağırmadan status'u denied sayıp hata veriyor (Android'de farklı başlangıç durumu yüzünden çalışıyordu). Cross-platform izin akışı bug'ı. İNCELENECEK — muhtemelen i2 ile aynı kök (request() çağrılmıyor).
- **(i5) Aşırı ısınma (NLLB/Gemma indirilmeden):** Performans. Şüpheli: mesh arka plan (4 AnimationController + tam ekran CustomPaint her frame repaint) + çok sayıda HgGlass `BackdropFilter` (saveLayer) + iPhone 120Hz ProMotion (`CADisableMinimumFrameDurationOnPhone=true` 120fps açıyor) → sürekli ağır GPU. İNCELENECEK — mesh repaint throttle / glass blur azalt.
